...
//                 and want to be right when using zzh's tb.
...
module conv5 #(
    ...
    input [kernel_size*kernel_size*channel_in*WIDTH-1:0] data_in,//数据的输入,432
    output reg valid,//输出有效信号
    output reg valid_ram,//输出有效信号
    output conv_stop,
    output reg [channel_out*WIDTH-1:0] data_out//数据输出,24bit
);
    wire [2:0] state;//状态机
    ...
    wire signed [2*WIDTH+3:0] pe_out_1;//输出数据:20bit
    wire signed [2*WIDTH+3:0] pe_out_2;//输出数据:20bit
    reg signed [3*WIDTH-2:0] data2bias_1;//输出数据:23bit
    reg signed [3*WIDTH-2:0] data2bias_2;//输出数据:23bit
    reg signed [3*WIDTH-2:0] data2bias_3;//输出数据:23bit
    reg signed [3*WIDTH-2:0] data2bias;//输出数据:23bit
    reg signed [3*WIDTH-2:0] data2bias_last;//输出数据:23bit
    ...
    always @(posedge clk or negedge rstn) begin
        ...
        else if(state==1&&cnt==2&&data_out_ok)
            valid_ram<=1;
        else valid_ram<=0;
    end
    //数据输出有效
    ...
    //         valid <= 0;
    //     end
    //     else if(state==1&&cnt==0&&data_out_ok)
    //         valid<=1;
    //     else valid<=0;
    // end
    always @(posedge clk or negedge rstn) begin
