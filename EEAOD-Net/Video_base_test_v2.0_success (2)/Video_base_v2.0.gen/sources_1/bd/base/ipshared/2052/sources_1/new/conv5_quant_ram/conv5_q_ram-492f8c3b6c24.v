module conv5_q_ram #(
    ...
    input [channel_in*WIDTH-1:0] data2ram_in , //data input 24bit
    output [kernel_size*kernel_size*channel_out*WIDTH-1:0] data2conv //data output 216bit
);
    ...
    reg [addr_width-1:0] cnt_row           =0      ;//读计数器（用于给行）,行的首地址
    reg [11:0] cnt_tile          =0      ;//块计数器（计算到本行滑窗到第几个3x3,每行滑窗最高为PIX_W-1个）
    ...
    assign cnt_row_full = (cnt_row==PIX_W*5)?1:0;
...
//读状态机
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            case (cstate)
                IDLE: if (addra==4*PIX_W+9) begin//改地址
                    nstate<=FIR_FIR;
                end
                else begin
                    nstate<=nstate;
                ...
                FIR_FIR:
                    nstate<=FIR_LOOP;
                FIR_LOOP:if (cnt1_full) begin//en加不加
                    nstate<=FIR_END;
                end
                else begin
                    nstate<=nstate;
                ...
                LOOP_FIR:
                    nstate<=LOOP_LOOP;
                LOOP_LOOP:if (cnt1_full) begin
                    nstate<=LOOP_END;
                end
                else begin
                    nstate<=nstate;
                end
                LOOP_END:if (loop2end) begin
                    nstate<=END_FIR;
                end
                else
                    nstate<=LOOP_FIR;
    ...
    */
    ...
    always @(posedge clk ) begin
        if(addrb[4]<PIX_W) begin
            addrb[1]<=addrb[4]+5*PIX_W;
            addrb[0]<=addrb[4]+5*PIX_W-1;
            addrb[2]<=addrb[4]+5*PIX_W+1;
        end
        else begin
    ...
    //下面
    always @(posedge clk ) begin
        if(addrb[4]>=5*PIX_W) begin
            addrb[7]<=addrb[4]-5*PIX_W;
            addrb[6]<=addrb[4]-5*PIX_W-1;
            addrb[8]<=addrb[4]-5*PIX_W+1;
        end
        else begin
